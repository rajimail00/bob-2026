...
const queryKey = (jobId: string) => ["messages", jobId];
...
export function useJobMessages(jobId: string) {
  ...
  const query = useQuery({
    ...
    queryFn: () => messagesApi.listForJob(jobId),
  });
  ...
  useEffect(() => {
    ...
    const onNewMessage = (message: Message) => {
      ...
      queryClient.setQueryData<Message[]>(queryKey(jobId), (current) => {
  ...
  const sendMessage = async (text: string) => {
    setIsSending(true);
    try {
      await messagesApi.send(jobId, { text });
    } finally {
      setIsSending(false);
    }
  };
